    ...
    async def _invoke(self, prompt: str) -> str | None:
        ...
        while True:
            ...
            if chat_resp.tool_calls:
                rprint("调用工具:", chat_resp.tool_calls)
...
async def example() -> None:
    ...
    try:
        ...
        #     f"爬取 https://news.ycombinator.com 的内容, 并且总结后保存在 {PROJECT_ROOT_DIR / 'llm+mcp+rag'/ 'output' / 'step3-agent-with-mcp'!s} 目录下的news.md文件中"
        # )
        resp = await agent.invoke(
            f"爬取 https://jsonplaceholder.typicode.com/users 的内容, 并且总结后保存在 {PROJECT_ROOT_DIR / 'knowledge' !s} 中，每个人创建一个.md文件"
        )
        rprint(resp)
